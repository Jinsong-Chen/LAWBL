...
2) **E-step:** Estimate with the GPCFA-LI model (E-step) by setting *LD=F*. Only a few loadings need to be specified in Q (e.g., 2 per factor). Longer chain is suggested for more accurate and stable estimation.
...
2) **E-step:** Estimate with the GPCFA-LI model (E-step) by setting *LD=FALSE*. Only a few loadings need to be specified in Q (e.g., 2 per factor). The first 12 items are categorical and need to be specified with *cati*.
